...
for tt= 1:length(T0_list)
    ...
    capital_cost_regHE = 0; % INo RegHE in Option 1 to 3; In Option 1 (Only Furnace), 2 (HE, HP, but losses in HE), 3. (HE, HP, no losses from HE); %$10,000
...
legend('Fan','Furnace (the whole heating process by Furnace)', 'Hospital Distribution','Overall System (Fan + ALL)')
title('Effect of Ambient Temperature Changes on Exergy Efficiency [%] - (Modified Option 2)')
xlabel('Outside or Ambient Tempearatue [C]')
ylabel('Exergy Efficiency [%]')

%% Saving Variables
if strcmpi(save_variables_option,'on')
    T0_list_option2 = T0_list;
    save('HeatingSystemData_option2.mat', ...
        'T0_list_option2', ...
...
filePath = 'Hospital_Project_Tables_Option_2.xlsx'; % Change this to your desired path
