...
T0_list =-10:2:10; %8;%-10:10;%8 %-10:2:10; % T0 = 8; % Ambient temperature (°C)
...
for tt= 1:length(T0_list)
  ...
  %% Modified _ exg_f_out_heatingwhole & Exg_f_rate_out_heatingwhole for Option 4 due to multiple inputs/outputs  
    ...
    Exg_Balance_Table_Option4_Fn(m_dot,Exg_rate_in_system,Exg_f_rate_in_fan,Exg_w_rate_in_fan,...
    Exg_q_rate_in_heatingwhole,Exg_q_rate_in_hp_free,...
    Exg_rate_disposed_system,Exg_q_rate_out_hospital,Exg_f_rate_out_hospital, ...
    Energy_q_rate_out_regHE, Exg_f_rate_out_regHE,... % Additional for Option 4 - RegHE
    Exg_q_rate_loss_furnace,Exg_q_rate_loss_he,...
    Exg_d_rate_fan,Exg_d_rate_heatingwhole,Exg_d_rate_hospital);

    %% Table - State Properties
    [State_Table, states, Specific_Energy, Carried_Energy_Rate, Specific_Flow_Exergy, Flow_Exergy_Rate] = States_Option4_Fn...
        (m_dot,cp,R,T0, T1, T2, T2A, T2B, T3, T4, T4A, P0, P1, P2, P2A, P2B, P3, P4, P4A);
...
%% Energy and Cost
close all;
plot(T0_list,Energy_in_fan_MJ_winter_list_Teffect,'b-', 'LineWidth',1.5); hold on % Add
plot(T0_list,Energy_in_heatingwhole_MJ_winter_list_Teffect,'r-', 'LineWidth',1.5);  %Add
legend('Energy into Fan [MJ]','Energy into Heatingwhole [MJ]')
xlabel('Outside or Ambient Tempearatue [C]')
ylabel('Energy [MJ]')
title('Effect of Ambient Temperature Changes on Energy Input')

figure

plot(T0_list,electricity_cost_fan_winter_list_Teffect,'b-o', 'LineWidth',1.5); hold on
plot(T0_list,fuel_cost_furnace_winter_list_Teffect, 'r-.d','LineWidth',1.5); 
plot(T0_list,total_cost_withCapital_period_winter_list_Teffect, 'k:','LineWidth',1.5); 

legend('Electricity Cost for Fan [USD]', 'Fuel Cost for Furnace [USD]','Total Cost including capital [USD]')
xlabel('Outside or Ambient Tempearatue [C]')
ylabel('Cost [USD]')
title('Effect of Ambient Temperature Changes on Energy and Cost')

%% Carbone Emissions
figure
plot(T0_list,electricity_fossil_CO2_emissions_winter_list_Teffect,'b-', 'LineWidth',1.5); hold on
plot(T0_list,natural_gas_CO2_emissions_winter_list_Teffect, 'r-.','LineWidth',1.5); hold on
plot(T0_list,total_CO2_emissions_winter_list_Teffect, 'k-.','LineWidth',1.5); 

legend('Electricity Fossil CO2 [kg]','Natural Gas CO2 Emissions [kg]', 'Total CO2 Emissions [kg]')
xlabel('Outside or Ambient Tempearatue [C]')
ylabel('CO2 Emissions [kg]')
title('Effect of Ambient Temperature Changes on Carbon Emissions')

%%
figure
%plot(T0_list,Energetic_efficiency_winter_list_Teffect,'b-', 'LineWidth',1.5); hold on
bar(T0_list,Energetic_efficiency_winter_list_Teffect(:,1:4),"grouped"); 
legend('Fan','Furnace (the whole heating process with HP and HE)', 'Hospital Distribution','Overall System (Fan + ALL)')
title('Effect of Ambient Temperature Changes on Energy Efficiency [%]')
xlabel('Outside or Ambient Tempearatue [C]')
...
figure
%plot(T0_list,Energetic_efficiency_winter_list_Teffect,'b-', 'LineWidth',1.5); hold on
bar(T0_list,Exergetic_efficiency_winter_list_Teffect(:,1:4),"grouped"); 
legend('Furnace (the whole heating process by Furnace)', 'Hospital Distribution','Overall System (Fan + ALL)')
title('Effect of Ambient Temperature Changes on Exergy Efficiency [%] (Modified Option -4)')
xlabel('Outside or Ambient Tempearatue [C]')
ylabel('Exergy Efficiency [%]')


%% Saving Variables
T0_list_option4 = T0_list;
save('HeatingSystemData_option4.mat', ...
    'T0_list_option4', ...
...
filePath = 'Hospital_Project_Tables_Option_4.xlsx'; % Change this to your desired path
